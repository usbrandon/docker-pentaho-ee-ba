...
if [ -f "/opt/pentaho/server/biserver-ee/start-pentaho.sh" ]; then
  #Upgrade Pentaho BA Server from version 6.0.1.0-386 to 6.0.1.1-398
...
# Unzip components, removing the archives as we go
  unzip -oq /tmp/pentaho/SP201601-6.0.zip -d /tmp/pentaho;
  ...
  # Upgrade pentaho BI to 6.0.1.1
  echo "Upgrading Pentaho BA from 6.0.1.0 to 6.0.1.1";
  unzip -oq /tmp/pentaho/SP201601-6.0/BIServer/SP201601-6.0-BIServer.zip -d $PENTAHO_HOME/server/biserver-ee/
  ...
  rm -rf /tmp/pentaho/SP201601-6.0*
fi